...
function listMessage(key) {
    ...
    var afterList = [];
    afterList.push("You can ask specific details about any of these models. eg: average rating of Galaxy S7." + " For comparing type 'compare Galaxy S7 and moto G'");
    afterList.push("You can ask for specs, ratings and prices across sites for these models. eg: specs of Moto G (or) camera specs of oneplus one." + " For comparing type 'compare Galaxy S7 and moto G'");
    afterList.push("You can ask specs of any model. eg: what is the processor of Moto X. For comparing type 'compare Galaxy S7 and moto G'" + " & to reset the conversation type 'clear'");
    ...
    if (key == 'notFound') {
        ...
        msg = "Sorry for that :( You can ask specific details of any model." + " eg: what is the processor of oneplus one";
        return msg;
    } else if (key == "help") {
...
function whyListMessage(key) {
    ...
    var send_msg = " These results are based on " + key + " scores from Phoneradar.com and buysmaart.com. Re-ranked them again after validating the scores from" + " digit.in, pcadvisor.com, 91mobiles.com, smartprix.com";
    return send_msg;
}
...
var useCaseAnswers = {
    ...
    gaming: {
        type: "checklist",
        max_select: 8,
        questions: [" High-end games (Asphalt, NFS most-wanted )", " Low-end games (Candycrush, stick cricket ) ", "I don't play games on mobile"],
    ...
    app_running: {
        type: "checklist",
        max_select: 8,
        questions: [" Social Networking apps (Eg: Facebook, Whatsapp, Snapchat)", " Low end gaming apps (Eg: Stick cricket, candycrush, mindgames)", " E-commerce Apps (Eg: Flipkart, Amazon, Myntra) ", " Entertainment apps (Eg: Netflix,hotstar, star sports) ", " High end gaming apps (Eg:Asphalt, Modern combat, FIFA)"],
    ...
    spend_time: {
        type: "checklist",
        max_select: 8,
        questions: ["Gaming ", " Watching movies", " Entertainment apps like Netflix,starsports, hotstar etc.,", " Chatting /Social Networking/E-commerce ", "others"],
    ...
    spend_time_battery: {
        type: "checklist",
        max_select: 8,
        questions: ["Gaming ", " Watching movies/youtube", " Social Networking/E-commerce/Browsing", " Listening to music ", " Phone calls/Chatting"],
        header_question: " You spend most of the time on phone on"
    ...
    frequent_use_activity: {
        type: "checklist",
        max_select: 8,
        questions: [" Watch videos/Youtube /movies ", " Reading/Heavy Browsing ", " Gaming ", " Food - ordering/E-commerce", " Chatting ", " Social Networking "],
    ...
    use_phone_light: {
        type: "checklist",
        max_select: 8,
        questions: ["Sun light (outdoor, travelling etc)", " Dim light  (in a room, in a room with light,night time etc) ", " Nothing in particular, anytime "],
        header_question: "You use your phone mostly in"
    },
    relevant_internal_memory: {
        type: "checklist",
        max_select: 8,
        questions: ["I play high-end games like Asphalt,Modern Combat, FIFA etc ", " I play low end games like candy crush, stick cricket, angry birds etc.", " I store a lot videos/Movies in my phone", " I store a lot of pics/music in my phone ", " I install more than 100 apps in my phone ", " I install less than 100 apps in my phone "],
        header_question: "Pick the ones that are most relevant to you"
    },
    micro_sd_card_cost: {
        type: "checklist",
        max_select: 1,
        questions: ["8 GB (Rs.150)", "16 GB (Rs.300)", "32 GB (Rs.500)", "64 GB (Rs.1200)", "128 GB (Rs. 2700)", "I don't want to buy any micro sd card. "],
...
function listFooter(key) {
    ...
    var msg;
    if (key == 0) msg = "These results are based on phone scores from Phoneradar.com and buysmaart.com. Re-ranked them again after validating the scores from" + " digit.in, pcadvisor.com, 91mobiles.com, smartprix.com ";else msg = "These results are based on the specs scores from popular mobile review sites like 91mobiles.com, pcadvisor.com, digit.in etc.";
    return msg;
}
...
function countMessages(question_count) {
    var message;
    if (question_count == 1) message = messageKeyValue("enquiry", "one");
    if (question_count == 2) message = messageKeyValue("enquiry", "two");
    if (question_count == 3) message = messageKeyValue("enquiry", "three");
    if (question_count == 4) message = messageKeyValue("enquiry", "four");
    if (question_count == 5) message = messageKeyValue("enquiry", "five");
    if (question_count == 6) message = messageKeyValue("enquiry", "six");
    if (question_count == 7) message = messageKeyValue("enquiry", "seven");
    if (question_count == 8) message = messageKeyValue("enquiry", "eight");
    if (question_count == 9) message = messageKeyValue("enquiry", "nine");
    return message;
}
module.exports = {
...
//# sourceMappingURL=messages-compiled-compiled.js.map